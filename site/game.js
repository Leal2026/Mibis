...
async function refreshDownloadCount(){
  let count=0;
  try{const response=await fetch("https://api.github.com/repos/Leal2026/Mibis/releases/tags/v1.1.1",{headers:{Accept:"application/vnd.github+json"}});if(response.ok){const data=await response.json(),asset=data.assets.find(item=>item.name==="Mibis.apk");count=Number(asset?.download_count)||0;}}
...
async function downloadGame(withDonation=false){
  ...
  await registerDownload();const link=document.createElement("a");link.href="https://github.com/Leal2026/Mibis/releases/download/v1.1.1/Mibis.apk";link.rel="noopener";document.body.appendChild(link);link.click();link.remove();
